...
export const howToConvertDocxToMarkdown: Guide = {
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        '그리고 그런 파일일수록 남의 서버에 올리고 싶지 않습니다. 내부 보고서, 계약서 초안, 회의 메모, 출간 전 원고 같은 문서 말입니다. CPdf는 파일이 이미 있는 곳에서 변환합니다 — 브라우저가 .docx를 읽고 .md를 디스크에 쓰며, 그 사이에 전송되는 것은 아무것도 없습니다. 탭을 닫으면 흔적도 남지 않습니다.',
      ],
    },
    {
  ...
  alternatives: [
    {
      ...
      paragraphs: [
        '모든 워드 프로세서는 문서를 .txt 파일로 저장할 수 있습니다. 무료이고 오프라인에서 되지만, 결과물에는 구조가 없습니다. 제목, 목록, 표가 전부 평평한 텍스트가 되어 손수 다시 만들어야 합니다. 짧은 문단이라면 괜찮지만, 문서 전체에는 CPdf의 구조 보존이 확실히 시간을 아껴 줍니다.',
      ],
    },
    {
      heading: '명령줄에서 Pandoc 사용',
      paragraphs: [
        'Pandoc은 .docx를 Markdown으로 네이티브 변환합니다(`pandoc report.docx -o report.md`). 배치 변환을 스크립트로 돌릴 때 적합한 도구입니다. 대신 설치가 필요하고 명령줄을 쓸 줄 알아야 하며, 복잡한 표에서 가끔 깔끔하지 않은 결과가 나올 수 있습니다. 브라우저에서 한 번만 변환한다면 CPdf는 파일 하나면 족합니다.',
      ],
    },
    {
  ...
  faqs: [
    {
      q: 'CPdf로 Word를 Markdown으로 변환하는 것은 무료인가요?',
      a: '네 — 모든 CPdf 도구는 워터마크, 일일 할당량, 프리미엄 등급 없이 영원히 무료입니다. 변환이 사용자 기기에서 실행되므로 서버 비용도 없습니다.',
    },
    {
      q: '어떤 서식이 보존되나요?',
