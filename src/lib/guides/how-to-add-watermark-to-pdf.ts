...
export const howToAddWatermarkToPdf: Guide = {
  ...
  intro:
    'PDF에 워터마크를 추가하는 가장 빠른 방법은 브라우저에서 바로 찍는 것입니다. 무료 [NoriPDF PDF 워터마크 도구](/watermark-pdf/)를 열고 문서를 끌어다 놓은 뒤, CONFIDENTIAL 같은 문구를 입력하거나 로고를 업로드하고, 타일 또는 가운데 배치를 선택한 다음 "워터마크 추가"를 클릭하면 watermarked.pdf로 결과를 다운로드할 수 있습니다. NoriPDF는 파일을 기기 안에서 처리하므로 워터마크가 없는 원본이 어디에도 업로드되지 않습니다. 계정도, 대기열도, 기다림도 없습니다.',
  quickSteps: [
  ...
  sections: [
    {
      ...
      image: {
        ...
        alt: 'NoriPDF PDF 워터마크 도구에 PDF가 추가되고 워터마크 종류가 텍스트로 선택된 화면. 워터마크 텍스트 입력란에는 CONFIDENTIAL이 입력되어 있음',
      },
    },
    {
      heading: '로고나 이미지로 찍기',
      ...
      image: {
        ...
        alt: 'NoriPDF PDF 워터마크 도구의 이미지 워터마크 옵션 화면. logo.png가 추가되고 이미지 크기 40%, 불투명도 20%, 바둑판식 배치가 선택되어 있음',
      },
    },
    {
      ...
      paragraphs: [
        ...
        'NoriPDF는 파일이 있는 바로 그곳에서 워터마크를 찍습니다. 문구는 기기의 폰트로 렌더링되어 브라우저 탭 안의 페이지 위에 그려지며, 어디에도 전송되거나 저장되지 않습니다. 개발자 도구의 네트워크 탭에서 업로드가 0건인 것을 확인할 수 있고, 탭을 닫으면 모든 흔적이 사라집니다.',
      ],
    },
    {
  ...
  alternatives: [
    {
      ...
      paragraphs: [
        '원본 문서가 아직 있다면 그 단계에서 워터마크를 넣으세요. Word에는 디자인 → 워터마크에 사용자 지정 텍스트와 그림이 있고, Google Docs에는 삽입 → 워터마크가 있습니다. 별도 비용 없이 할 수 있지만, 직접 만든 문서에만 해당됩니다. 스캔본이나 PDF로 받은 계약서에는 원본 파일이 없습니다. NoriPDF는 실제로 가지고 있는 그 문서에 바로 찍어 줍니다.',
      ],
    },
    {
  ...
  faqs: [
    {
      q: 'NoriPDF로 워터마크를 추가하는 것은 정말 무료인가요?',
      a: '네 — 영원히 무료이며 일일 할당량도, 유료 플랜도 없습니다. NoriPDF는 자체 브랜딩을 추가하지 않습니다. 워터마크는 오직 직접 넣은 것만 존재합니다.',
    },
    {
