...
export const howToConvertJpgToPdf: Guide = {
  ...
  intro:
    'JPG 이미지를 PDF로 변환하는 가장 빠른 방법은 브라우저에서 바로 처리하는 것입니다. 무료 [NoriPDF JPG를 PDF로 도구](/jpg-to-pdf/)를 열고, JPG 또는 PNG 이미지를 최대 20장 추가한 뒤 페이지 방향과 배치를 선택하고, "PDF로 변환"을 클릭하면 결과를 images.pdf로 다운로드할 수 있습니다. 변환은 기기에서 로컬로 실행되므로 사진이 서버에 업로드되지 않습니다. 계정도, 대기열도, 워터마크도 없습니다.',
  quickSteps: [
  ...
  sections: [
    {
      heading: '단계별 안내: NoriPDF로 이미지 변환하기',
      paragraphs: [
      ...
      image: {
        ...
        alt: 'NoriPDF JPG를 PDF로 도구에 이미지 파일 네 개가 추가된 화면. 각 항목에 파일 이름, 크기, 제거 버튼이 표시되어 있다.',
      },
    },
    {
    ...
    },
    {
      ...
      paragraphs: [
        ...
        'NoriPDF는 업로드 없이 같은 일을 합니다. 브라우저가 이미지를 읽고 A4 페이지에 배치한 뒤 PDF를 내 디스크에 저장합니다. 전송되는 것이 아무것도 없어 신분증이나 NDA 자료에도 안전하며, 페이지를 불러온 뒤에는 오프라인에서도 작동합니다. 휴대폰 사진은 별도로 신경 씁니다. EXIF 방향 태그를 자동으로 적용해 세로로 찍은 사진이 눕지 않습니다.',
      ],
    },
    {
  ...
  alternatives: [
    {
      ...
      paragraphs: [
        'Windows는 별도 소프트웨어 없이도 이미지를 변환할 수 있습니다. 파일 탐색기에서 JPG를 선택하고 마우스 오른쪽 버튼을 클릭한 뒤 인쇄를 선택하고, Microsoft Print to PDF 프린터에 페이지를 꽉 채우는 사진 레이아웃을 지정하세요. 무료이며 오프라인에서 되지만 레이아웃 조절은 기본적이고 순서는 선택한 순서를 따릅니다. NoriPDF는 동일한 오프라인 프라이버시에 방향과 맞춤 조절을 명확하게 제공합니다.',
      ],
    },
    {
      heading: 'macOS 미리보기(Preview)',
      paragraphs: [
        'Mac에서는 이미지를 미리보기에 함께 연 뒤 파일 → 인쇄 → PDF로 저장을 선택하세요. 무료이고 오프라인에서 되지만 macOS에서만 되며, 여백이나 빈틈없는 인쇄 옵션은 인쇄 대화상자 안에 숨어 있습니다. NoriPDF는 브라우저라면 어디서나 같은 일을 하며 맞춤 옵션이 바로 보입니다.',
      ],
    },
    {
  ...
  faqs: [
    {
      q: 'NoriPDF로 JPG를 PDF로 변환하는 것은 정말 무료인가요?',
      a: '네. NoriPDF의 모든 도구는 영원히 무료이며 일일 한도, 워터마크, 유료 요금제가 없습니다. 변환은 내 기기에서 실행되므로 서버 비용을 사용자에게 전가할 일이 없습니다.',
    },
    {
