...
export const howToMergePdf: Guide = {
  ...
  intro:
    'PDF 파일을 온라인에서 병합하는 가장 간단한 방법은 브라우저에서 바로 결합하는 것입니다. 무료로 사용할 수 있는 [NoriPDF PDF 병합 도구](/merge-pdf/)를 열고, 2~20개 파일을 드래그 앤 드롭으로 추가한 뒤, 원하는 순서로 배열하고, PDF 병합 버튼을 클릭하면 결과물이 merged.pdf로 다운로드됩니다. 대부분의 온라인 병합 도구와 달리 NoriPDF는 모든 처리를 기기 내에서 수행하므로 문서가 서버에 업로드되지 않으며, 계정도 대기열도 워터마크도 없습니다.',
  quickSteps: [
  ...
  sections: [
    {
      heading: '단계별 안내: NoriPDF로 PDF 병합하기',
      paragraphs: [
      ...
      image: {
        ...
        alt: '파일 이름과 크기, 위아래 이동 및 삭제 버튼이 표시된 여러 PDF 파일이 추가된 NoriPDF PDF 병합 업로드 영역',
      },
    },
    {
      ...
      paragraphs: [
        ...
        'NoriPDF는 이 모델을 뒤집습니다: 병합 소프트웨어가 브라우저로 전달되고 파일은 기기를 떠나지 않습니다. 전송도 대기열도 외부 저장도 없으므로, 회사 노트북에서나 공공 Wi-Fi에서, 또는 NDA가 적용되는 문서를 다룰 때도 안심하고 쓸 수 있습니다. 페이지를 불러온 뒤 인터넷 연결을 끊어도 병합이 그대로 작동합니다. 탭을 닫으면 문서의 흔적이 전혀 남지 않습니다.',
      ],
    },
    {
  ...
  alternatives: [
    {
      ...
      paragraphs: [
        'Mac에서는 Preview로 추가 소프트웨어 없이 PDF를 병합할 수 있습니다: PDF를 열고 썸네일 사이드바를 표시한 뒤, 다른 PDF를 썸네일 목록으로 드래그한 뒤 파일 → PDF로 내보내기를 선택해 저장하세요. 무료이며 오프라인에서 작동하지만 macOS 전용이고, 썸네일을 드래그해 많은 페이지를 재정렬하다 보면 번거로워집니다. NoriPDF는 브라우저만 있다면 모든 운영체제에서 동일한 오프라인 프라이버시를 제공하며, 화살표 버튼으로 정확한 순서 지정이 가능합니다.',
      ],
    },
    {
      heading: 'Windows: PDF 인쇄 우회 방법',
      paragraphs: [
        'Windows에는 기본 제공 PDF 병합 기능이 없습니다. 흔한 우회 방법은 Microsoft Print to PDF 가상 프린터를 쓰는 것입니다: 문서를 인쇄할 수 있는 앱에서 함께 열고, 결과를 새 PDF로 인쇄하거나 저장합니다. 급할 때는 쓸 만하지만, 인쇄 과정에서 모든 것이 다시 렌더링되어 링크, 양식 필드, 북마크가 평탄화됩니다. NoriPDF 같은 직접 병합 도구가 더 빠르고 무손실입니다.',
      ],
    },
    {
  ...
  faqs: [
    {
      q: 'NoriPDF로 PDF 병합은 정말 무료인가요?',
      a: '네 — NoriPDF의 모든 도구는 무료로 영구 이용 가능하며, 일일 제한, 워터마크, 프리미엄 요금제가 없습니다. 병합은 사용자 기기에서 실행되므로 서버 비용이 발생하지 않아 사용자에게 전가될 일도 없습니다.',
    },
    {
    ...
    },
    {
      ...
      a: '아니요. NoriPDF는 페이지를 객체 수준에서 새 문서로 복사하므로 재압축도 재렌더링도 없습니다. 텍스트는 선명하고 선택 가능한 상태를 유지하고, 이미지는 원본 해상도를 그대로 유지하며, 결과물은 어디서나 열리는 일반 PDF입니다.',
    },
  ],
