...
export const howToExtractPagesFromPdf: Guide = {
  ...
  intro:
    'PDF에서 페이지를 추출하는 가장 간단한 방법은 브라우저에서 직접 눈으로 골라 추출하는 것입니다. 무료 [NoriPDF 페이지 추출 도구](/extract-pages/)를 열고 파일을 놓으면, 원하는 페이지의 썸네일을 클릭하기만 하면 됩니다 — 챕터 한 개, 서명 페이지, 여러 장 중 인보이스 한 장 등. 그 페이지만 extracted.pdf로 다운로드됩니다. 모든 처리는 기기 안에서 이루어지며 문서는 어디에도 업로드되지 않습니다.',
  quickSteps: [
  ...
  sections: [
    {
      heading: '단계별: NoriPDF로 PDF 페이지 추출하기',
      paragraphs: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        'NoriPDF는 선택과 재구성 전 과정을 브라우저 안에서 처리합니다. 원본 파일은 내 디스크에서 읽히고 새 PDF도 그곳에 저장됩니다. 계정도 없고 서버에 복사본도 없으며 탭을 닫으면 흔적도 남지 않습니다.',
      ],
    },
    {
  ...
  alternatives: [
    {
      ...
      paragraphs: [
        'Mac에서 Preview로 PDF를 열고 썸네일을 표시한 뒤 원하는 페이지를 선택하고 파일 → 클립보드에서 새로 만들기(또는 바탕화면으로 드래그)로 새 문서에 복사하세요. 무료이며 오프라인에서 동작하지만 macOS에서만 가능하고 페이지가 많은 문서에는 불편합니다. NoriPDF는 어떤 브라우저에서도 동일한 로컬 사생활 보호를 제공합니다.',
      ],
    },
    {
      heading: 'PDF로 인쇄(페이지 범위)',
      paragraphs: [
        '특정 페이지 범위만 새 PDF로 인쇄하는 것은 흔한 우회 방법이지만, 인쇄는 모든 것을 다시 렌더링합니다. 링크, 양식 필드, 북마크가 평탄화되고 품질이 저하될 수 있습니다. NoriPDF의 추출은 원본 페이지를 있는 그대로 무손실로 복사합니다.',
      ],
    },
  ],
  edgeCases: [
