...
export const howToOrganizePdfPages: Guide = {
  ...
  intro:
    'PDF 페이지의 순서를 바꾸거나 삭제, 회전하는 가장 빠른 방법은 브라우저에서 바로 처리하는 것입니다. 무료 [NoriPDF PDF 페이지 정리 도구](/organize-pdf/)를 열고 파일을 놓은 뒤, 페이지 썸네일을 원하는 위치로 드래그하고, 옆으로 누운 페이지는 회전시키고, 필요 없는 페이지는 삭제하세요. "정리된 PDF 다운로드"를 클릭하면 organized.pdf로 결과를 저장합니다. 모든 처리는 기기에서 로컬로 이루어지며, 파일이 업로드되지 않고 계정이나 대기열, 워터마크도 없습니다. 인터넷 연결 없이도 동작하고, 원본 파일은 전혀 변경되지 않습니다.',
  quickSteps: [
  ...
  sections: [
    {
      heading: '단계별 안내: NoriPDF로 페이지 순서 바꾸기',
      paragraphs: [
      ...
      image: {
        ...
        alt: '문서를 불러온 뒤 NoriPDF PDF 페이지 정리 그리드에 페이지 썸네일이 표시된 화면. 그리드 위에 선택 도구 모음, 오른쪽에 유지 페이지 수 카운터가 보임',
      },
    },
    {
    ...
    },
    {
      ...
      paragraphs: [
        ...
        'NoriPDF는 미리보기 렌더링과 페이지 재배치를 전부 브라우저 안에서 처리합니다. 파일은 사용자 기기의 디스크에서 읽히고, 다시 작성된 PDF은 그대로 기기에 저장됩니다. 업로드도 대기열도 임시 복사본도 없습니다. 페이지를 불러온 뒤 인터넷 연결을 끊어도 동작하며, 탭을 닫으면 문서의 흔적이 남지 않습니다.',
      ],
    },
    {
  ...
  alternatives: [
    {
      ...
      paragraphs: [
        'Mac에서는 미리보기(Preview)로 세 가지 작업을 모두 처리할 수 있습니다. 보기 → 미리보기 표시를 켜고, 드래그로 순서를 바꾸고, 페이지를 선택해 Delete 키를 누르고, 도구 → 왼쪽/오른쪽으로 회전을 사용하세요. 무료이며 오프라인에서 동작하지만 macOS에서만 사용할 수 있고, 문서를 직접 수정하므로 저장하는 순간 삭제가 영구 적용됩니다. NoriPDF는 같은 오프라인 프라이버시를 모든 브라우저에서 제공하며, 복원 기능이 있어 안심하고 시도해 볼 수 있습니다.',
      ],
    },
    {
      heading: 'Adobe Acrobat(유료)',
      paragraphs: [
        'Acrobat의 페이지 구성 보기는 완성도 높은 유료 표준입니다. 그리드에서 썸네일을 드래그하고, 회전, 삭제, 삽입, 추출을 한 화면에서 처리할 수 있습니다. 하지만 구독과 데스크톱 설치가 필요하고, 웹 버전은 파일을 Adobe 서버에 업로드합니다. 이미 Acrobat을 구독 중이라면 훌륭한 선택입니다. 그렇지 않다면 NoriPDF로 계정 없이, 업로드 없이 같은 작업을 처리할 수 있습니다.',
      ],
    },
    {
  ...
  faqs: [
    {
      q: 'NoriPDF로 PDF 페이지 정리는 정말 무료인가요?',
